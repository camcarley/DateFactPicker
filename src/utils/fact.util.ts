 ...
 * @returns a
 */
export const fetchFact = async (dayNumber: number, monthNumber: MonthNumber) => {
	const res: Response = await fetch(`http://numbersapi.com/${monthNumber}/${dayNumber}/date?json`, {
		method: "GET",
	});
	if (!res?.ok) {
		return {} as Fact;
